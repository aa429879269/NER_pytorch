...
from CRF import CRF
import os
model_path = "model_bert_BiLSTM2_LSTM_CRF"
batch_size = 96
...
from word_bert_vector import get_bert_vector
import numpy
def inference():
    net.load_state_dict(torch.load(f'{model_path}/model_74.pth'))
    while True:
        sentence = input('句子')
        x = ['BIN']+list(sentence)+['EOS']
        train_x_ = get_bert_vector(x)
...
def inference_no_start_bert_server():
    ...
    while True:
        index = input('句子0/22/23/.....(都是测试集的句子)')
